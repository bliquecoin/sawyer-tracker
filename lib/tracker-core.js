(function (root, factory) {
  ...
  function seizureEpisodeGaps(seizures, windowMs = 24 * 60 * 60 * 1000) {
    const active = seizures
      .filter((event) => event.type === "seizure" && !event.deletedAt)
      .sort((a, b) => new Date(a.occurredAt) - new Date(b.occurredAt));
    const episodes = [];

    active.forEach((seizure) => {
      const currentEpisode = episodes.at(-1);
      const previousSeizure = currentEpisode?.at(-1);
      if (previousSeizure && seizuresShareClusterWindow(previousSeizure, seizure, windowMs)) {
        currentEpisode.push(seizure);
      } else {
        episodes.push([seizure]);
      }
    });

    const gaps = [];
    for (let index = 1; index < episodes.length; index += 1) {
      const from = episodes[index - 1].at(-1);
      const to = episodes[index][0];
      gaps.push({
        from,
        to,
        days: (new Date(to.occurredAt) - new Date(from.occurredAt)) / 86400000
      });
    }
    return gaps;
  }
